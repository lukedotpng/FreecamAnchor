...
void FreecamAnchor::OnEngineInitialized() {
    Logger::Info("FreecamAnchor has been initialized!");
...
void FreecamAnchor::OnDrawMenu() {
    ...
    if (ImGui::Button(ICON_MD_LOCAL_FIRE_DEPARTMENT " FreecamAnchor")) {
...
void FreecamAnchor::OnDrawUI(bool p_HasFocus) {
    if (m_ShowMessage) {
        ...
        if (ImGui::Begin("FreecamAnchor", &m_ShowMessage)) {
            // Only show these when the window is expanded.
            ImGui::Text("Hello from FreecamAnchor!");
        }
        ImGui::End();
    }
}
...
DEFINE_PLUGIN_DETOUR(FreecamAnchor, void, OnLoadScene, ZEntitySceneContext* th, ZSceneData& p_SceneData) {
